...
class MNIST:
    ...
    def __init__(self, model_path=None, data_path=None, has_unknown=False, drop_out=None):
        ...
        self.Y = tf.placeholder(tf.float32, [None, self.num_output])
        self.p_keep_conv = tf.placeholder(tf.float32)
        self.p_keep_hidden = tf.placeholder(tf.float32)
    ...
    def build_cnn_model(self, p_keep_conv=1., p_keep_hidden=1.):
        ...
        W5 = TFUtils.xavier_init([625, self.num_output], 'W5')
        ...
        # Output(labels) FC 625 inputs -> 11 outputs
    ...
    def save_model(self):
        if self.model_path is not None:
            ...
            name = os.path.basename(self.model_path) + '-checkpoint'
            saver.save(self.sess, self.model_path, latest_filename=name)
    ...
    def build_accuracy(self):
        ...
        self.accuracy = tf.reduce_mean(tf.cast(check_prediction, tf.float32))

    def check_accuracy(self, test_feed_dict=None):
        accuracy_rates = self.sess.run(self.accuracy, feed_dict=test_feed_dict)
